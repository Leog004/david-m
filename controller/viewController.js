...
exports.postContact = catchAsync(async (req, res) => {
  ...
  if (!name || !email || !message) {
    return res.redirect('/contact?status=fail');
  }

  try {
    await emailController.sendEmail(name, email, message, subject);
...
exports.verifyRecaptcha = catchAsync(async (req, res) => {
  ...
  if (userIp.includes(',')) {
    userIp = userIp.split(',')[0]; // Get the first IP in the list in case of multiple IPs
  }
